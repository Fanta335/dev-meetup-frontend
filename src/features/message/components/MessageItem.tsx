...
export const MessageItem: FC<Props> = memo(
  ({ messageId, virtualListId, handleClickReply }: Props) => {
    ...
    return (
      <>
        <Box sx={[{ px: 1 }, { '&:hover': { bgcolor: '#00000013' } }]}>
          {parentMessageId && (
            <ReplyAccessory parentMessageId={parentMessageId} handleClickReply={handleClickReply} />
          )}
        </Box>
        <Box
          sx={
            isReplyingToThisMessage ? { bgcolor: '#1100ff2f', borderLeft: '5px solid #772CE8' } : {}
          }
        >
          <Box
            component="div"
